...
void main() {
    ...
    vec4 noiseX = bccNoisePlaneFirst(vec3(uv.x * frequency, uv.y * frequency, u_Time * timeMult));
    vec4 noiseY = bccNoisePlaneFirst(vec3(uv.x * frequency, uv.y * frequency, u_Time * timeMult));
    
    // Test noise (kinda like turbulence?)
    vec4 testNoise = abs(vec4(noiseX.x + noiseX.y + noiseX.z) + vec4(noiseY.x + noiseY.y + noiseY.z));
    testNoise *= 0.1;

    // Ridges only
    vec4 ridgeNoise = 1.0 - (vec4(noiseX.x + noiseX.y + noiseX.z) * vec4(noiseY.x + noiseY.y + noiseY.z));


    // Sharp valleys ridge noise
