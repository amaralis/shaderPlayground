...
void main() {
    ...
    float timeNoise1 = pnoise(vec4(vec2(uv.x * scale * applyFrequency(lacunarity, 0.0), uv.y * scale * applyFrequency(lacunarity, 0.0)), time + sin(uv.x), time + cos(uv.y)), vec4(1.0)) * applyAmplitude(persistence, 0.0);
    ...
    vec4 noise1 = vec4(bccNoisePlaneFirst(vec3(offset * 1.0 + uv.x * applyFrequency(lacunarity, 0.0), offset * 1.0 + uv.y * applyFrequency(lacunarity, 0.0), timeNoise1))) * applyAmplitude(persistence, 0.0);
